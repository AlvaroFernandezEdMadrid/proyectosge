...
from ClaseCorreo import MandarEmail
...
class Ventana(tk.Tk):
    ...
    def generarPDF(self, username):
        ...
        print(puntuacion)
        print(top10)
        pdf_creator.render_pdf(f'/home/usertar/Escritorio/{username}_prueba.pdf', context)
    ...
    def mainFrames(self):
        ...
        btnMainMandarCorreo=self.genButton(self.sidebar,"Mandar al correo",lambda:threading.Thread(target=self.enviarCorreo(self.nickname)).start(),5,10,"center")
        
        btnMainMostrarGrafica = self.genButton(self.sidebar,"Mostrar gráfica",lambda: self.mostrarGrafica(),5, 10, "center")
        
        btnMainGenerarPDF=self.genButton(self.sidebar,"Generar PDF",lambda:threading.Thread(target=self.generarPDF(self.nickname)).start(),5,10,"center")
